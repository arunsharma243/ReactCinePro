...
import { fetchTopRatedMovies, fetchUpcomingMovies, image342 } from '../api/moviedb';
...
const TopRatedMovies = () => {
  ...
  const getTopRatedMovies = async (page) => {
    ...
    if (data && data.results) {
      setTopRated(prev => [...prev, ...data.results]); // Append new data
...
<TouchableOpacity onPress={()=>navigation.goBack()} style={[styles.background,{borderRadius:10}]}>
  ...
  <Text style={styles.headerText}>
    <Text style={styles.logoText}>T</Text>op Rated
  </Text>
